...
import numpy as np
import os
from utils import count_correct, get_batches, get_predictions_from_sequences, f1_per_class
...
class AutoencodeSeq:
    ...
    def build_graph(self):
        ...
        self.total_loss = self.reconstruction_loss + self.sequence_loss
    ...
    def fit(self,
        ...
        validation = True
        if np.sum([1 if (x is None) else 0 for x in [test_data, test_data_mask, test_data_orders]]) \
                > 0:
            validation = False
            ...
            with tf.Session() as sess:
                ...
                for epoch in range(n_epochs):
                    ...
                    for rows in get_batches(list(range(data.shape[0])), batch_size=batch_size):
                        ...
                        if self.use_regularization:
                            _, rec_loss, reg_loss, seq_loss, step = sess.run(
                                [train_step, self.reconstruction_loss, self.regularization_loss,
                            ...
                            _, rec_loss, seq_loss, step = sess.run(
                                [train_step, self.reconstruction_loss, self.sequence_loss,
                                 global_step],
                                feed_dict={
                                    self.input_: rows_features,
                                    self.input_mask: rows_masks,
                    ...
                    if epoch % print_every_epochs == 0:
                        if verbose and validation:
                            ...
                            if self.use_regularization:
                                print('At epoch {:4d} rec_loss: {:8.4f} reg_loss: {:8.4f} '
                                      'seq_loss {:8.4f}'
                                      .format(epoch, reconstruction_loss, regularization_loss,
                                              sequence_loss), end='')
                            else:
                                print('At epoch {:4d} rec_loss: {:8.4f} '
                                      'seq_loss {:8.4f}'
